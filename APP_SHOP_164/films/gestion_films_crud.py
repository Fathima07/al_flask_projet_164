...
from APP_SHOP_164.films.gestion_films_wtf_forms import FormWTFUpdateFilm, FormWTFAddFilm, FormWTFDeleteFilm
    ...
    if request.method == "POST":
        try:
            if form_add_film.validate_on_submit():
                ...
                strsql_insert_film = """INSERT INTO t_products (products_id,product_title) VALUES (NULL,%(value_nom_film)s) """
...
def film_update_wtf():
    ...
    try:
        ...
        if form_update_film.validate_on_submit():
            ...
            str_sql_update_nom_film = """UPDATE t_products SET product_title = %(value_nom_film)s,
                                                            product_desc = %(value_duree_film)s,
                                                            product_price = %(value_description_film)s,
                                                            product_image1 = %(value_cover_link_film)s                 
                                                            WHERE products_id = %(value_id_film)s"""
            with DBconnection() as mconn_bd:
            ...
            str_sql_id_film = "SELECT * FROM t_products WHERE products_id = %(value_id_film)s"
            ...
            print("data_film ", data_film, " type ", type(data_film), " genre ",
                  data_film["product_title"])

            # Afficher la valeur sélectionnée dans le champ du formulaire "film_update_wtf.html"
            form_update_film.nom_film_update_wtf.data = data_film["product_title"]
            form_update_film.duree_film_update_wtf.data = data_film["product_desc"]
...
def film_delete_wtf():
    ...
    try:
        ...
        if form_delete_film.submit_btn_del_film.data:
            ...
            str_sql_delete_fk_film_genre = """DELETE FROM t_brand_product WHERE fk_product = %(value_id_film)s"""
            str_sql_delete_film = """DELETE FROM t_products WHERE products_id = %(value_id_film)s"""
        ...
        if request.method == "GET":
            ...
            str_sql_genres_films_delete = """SELECT * FROM t_products WHERE products_id = %(value_id_film)s"""
