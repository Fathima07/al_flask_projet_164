...
from APP_SHOP_164.database.database_tools import DBconnection
from APP_SHOP_164.erreurs.exceptions import *
    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT products_id, product_title, product_desc, product_image1, product_price,
                                                            GROUP_CONCAT(brand_title) as GenresFilms FROM t_brand_product
                                                            RIGHT JOIN t_products ON t_products.products_id = t_brand_product.fk_product
                                                            LEFT JOIN t_brands ON t_brands.brand_id = t_brand_product.fk_brand
                ...
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING products_id = %(value_id_film_selected)s"""
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT brand_id, brand_title FROM t_brands ORDER BY brand_id ASC"""
            ...
            lst_data_film_selected = [item['products_id'] for item in data_genre_film_selected]
            ...
            lst_data_genres_films_non_attribues = [item['brand_id'] for item in data_genres_films_non_attribues]
            ...
            lst_data_genres_films_old_attribues = [item['brand_id'] for item in data_genres_films_attribues]
            ...
            lst_data_genres_films_non_attribues = [item['brand_title'] for item in data_genres_films_non_attribues]
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_brand_product (id_brand_pro, fk_product, fk_brand)
            ...
            strsql_delete_genre_film = """DELETE FROM t_brand_product WHERE fk_brand = %(value_fk_genre)s AND fk_product = %(value_fk_film)s"""
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT products_id, product_title, product_desc, product_image1, product_price,GROUP_CONCAT(brand_id) as GenresFilms FROM t_brand_product
                                        INNER JOIN t_products ON t_products.products_id = t_brand_product.fk_product
                                        INNER JOIN t_brands ON t_brands.brand_id = t_brand_product.fk_brand
                                        WHERE products_id = %(value_id_film_selected)s"""

        strsql_genres_films_non_attribues = """SELECT brand_id, brand_title FROM t_brands WHERE brand_id not in(SELECT brand_id as idGenresFilms FROM t_brand_product
                                                    INNER JOIN t_products ON t_products.products_id = t_brand_product.fk_product
                                                    INNER JOIN t_brands ON t_brands.brand_id = t_brand_product.fk_brand
                                                    WHERE products_id = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT products_id, brand_id, brand_title FROM t_brand_product
                                            INNER JOIN t_products t_products.products_id = t_brand_product.fk_product
                                            INNER JOIN t_brands ON t_brands.brand_id = t_brand_product.fk_brand
                                            WHERE products_id = %(value_id_film_selected)s"""
